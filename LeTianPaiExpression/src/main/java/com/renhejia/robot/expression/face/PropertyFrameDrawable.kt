...
class PropertyFrameDrawable(res: Resources?, `is`: InputStream?) :
    ...
    private inner class SubFrameDrawable {
        ...
        fun initScaleAnimation(range: Array<Any>, ratio: FloatArray) {
            logi("scale", "initScaleAnimation ====== 1  ")
            val holder = createValuesHolder("scale", range, ratio)
            logi("scale", "initScaleAnimation ====== 2  ")
            scaleAnimation =
                ValueAnimator.ofPropertyValuesHolder(holder) //.ofObject(new FaceFloatEvaluator(ratio), range);
            logi("scale", "initScaleAnimation ====== 3  ")
            scaleAnimation!!.setDuration(duration.toLong())
            logi("scale", "initScaleAnimation ====== 4  ")
            if (isOneShot) {
                logi("scale", "initScaleAnimation ====== 5  ")
                scaleAnimation!!.setRepeatCount(ValueAnimator.INFINITE)
            }
            scaleAnimation!!.setInterpolator(LinearInterpolator())
            logi("scale", "initScaleAnimation ====== 6  ")
            scaleAnimation!!.addUpdateListener(ValueAnimator.AnimatorUpdateListener { animation ->
                scale = animation.animatedValue as Scale
                logi(
                    "scale",
                    "initScaleAnimation ====== 7  "
                )
                logi("scale", "scale.x: " + scale.x)
                logi("scale", "scale.y: " + scale.y)
                updateMatrix()
                needInvalidate()
            })
            items.add(scaleAnimation!!)
            logi("scale", "initScaleAnimation ====== 8  ")
        }
        ...
        fun initPivotAnimation(range: Array<Any>, ratio: FloatArray) {
            ...
            piovtAnimation!!.setInterpolator(LinearInterpolator())
        ...
        fun initRotateAnimation(range: Array<Any>, ratio: FloatArray) {
            ...
            degreesAnimation!!.setInterpolator(LinearInterpolator())
        ...
        fun updateMatrix() {
            ...
            matrix.setScale(scale.x, scale.y, px.toFloat(), py.toFloat())
            val dx = piovt!!.x - px
            val dy = piovt!!.y - py
