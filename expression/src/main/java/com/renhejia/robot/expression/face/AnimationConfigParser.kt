...
class AnimationConfigParser {
    ...
    fun parse(`is`: InputStream?): List<FaceFrame?>? {
        ...
        val handler: FrameHandler = FrameHandler()
    ...
    private inner class FrameHandler : DefaultHandler() {
         ...
         * @param ratio
         */
        fun addStep(mode: Int, range: Float, ratio: Float) {
            when (mode) {
                Companion.STEP_MODE_SCALE ->                // 缩放
                    subFrame!!.addScaleStep(range, ratio)

                Companion.STEP_MODE_ALPHA ->                // 透明度
                    subFrame!!.addAlphaStep(range, ratio)

                Companion.STEP_MODE_ROTATE ->                // 旋转
                    subFrame!!.addRotateStep(range.toInt(), ratio)
            }
        }
