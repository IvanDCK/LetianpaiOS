...
//import com.letianpai.robot.letianpaiservice.ILetianpaiService

class BleService : Service() {
    private var ancsService: ANCSService? = null
    ...
    private fun readDataFromDevice(bleData: String?) {
        if (bleData != null && bleData.length > 3) {
            val cmd = getCmd(bleData)
            val data = getData(bleData)
            LogUtils.logd(
                "BleService",
                "收到蓝牙消息：cmd: $cmd  data: $data"
            )
            when (cmd) {
                BleConsts.BLE_CMD_MESSAGE_TIPS -> {
                    sendMessageTips(data)
                }

                BleConsts.BLE_CMD_COMMON_CONTROL -> {
            ...
            LogUtils.logd(
                "BleService",
                "readDataFromDevice: 收到消息没有进行分发：$bleData"
            )
        }
    }
     ...
     * @param data
     */
    private fun sendCommonControl(data: String) {
        when (data) {
            "shutdown" -> {
                shutdownRobot(this)
            }
            ...
            "start_ancs" -> {
                Log.d("<<<<", "启动iOS_ANCS")
                ancsService = ANCSService(this@BleService)
                //ios 来的通知
                ancsService!!.setBleDataListener { bleData: String? ->
                    this@BleService.readDataFromDevice(
                        bleData
                    )
                }
            }
    ...
    private fun sendMessageTips(data: String) {
        if (iLetianpaiService != null) {
            try {
...
//                String m = "{\"title\":\"title\",\"msg\":\"" + data + "\",\"time\":\"2023-06-28 22:51:49\",\"package\":\"com.tencent.mobileqq\"}";
    ...
    private fun getCmd(bleData: String): String {
        return bleData.substring(0, 3)
    }

    private fun getData(bleData: String): String {
    ...
    private val ltpBleResponseCallback = object : LtpBleResponseCallback.Stub() {
        override fun onBleCmdResponsReceived(command: String, data: String) {
            try {
                writeDataToDevice(command, data)
            } catch (e: RemoteException) {
                Log.e("LtpBleResponseCallback", "RemoteException: ${e.message}", e)
            }
        }
    }
    ...
    private fun changeRomLanguage() {
        val locale = Locale("zh") //en
        try {
